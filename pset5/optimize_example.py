...
# minimizes the function you provide.
#
# Because SciPy `optimize.minimize` has to work on a wide variety of
# functions, the calling interface to your function has to be very
# specific, so SciPy can call it generically.
#
# The first argument to your objective function is an ndarray
# containing the current values of your parameters (that you're trying
# to optimize). The first thing your f(p) function might do is unpack
# this ndarray and name the different parameters.
#
...
# tuple in the `args` optional argument, and SciPy passes them on
# (separately) to your objective function every time it needs to call
# it. Here, this is just a single extra argument: <D>, the data.
#
...
# interface, you also need a first `guess`: the initial p0 vector that you'll
...
def alt_nll(p, D):
    ...
    for x in D:
        ...
        ll      += stats.norm.logpdf(residual, 0, curr_sigma)    
        # Alternatively, using stats.norm.logpdf(x, curr_mu, curr_sigma) is shorter & equivalent,
        # but doing it in terms of the residual is a more direct analog to this week's problem set.
    return -ll
...
# exp(lambda) can overflow easily. 
...
mu_alt    = alt_result.x[0]
sigma_alt = np.exp(alt_result.x[1])
